...
Ahhh! It only shows the first two words- not the third! The full name is Ki-Adi-Mundi, and it only shows Ki-Adi.
...
##Sidenote: I am LOVING this exercise so far. This is SUCH an awesome way to play around with new, functional code and explore how it actually works. Thank you so much for putting this together!!
...
sw_skin_sep <- starwars %>% #I'm starting from the starwars data
  separate(col = skin_color, #This command separates the 'skin_color' column
           into = c("skin_1", "skin_2"), #into two columns, skin_1 and skin_2
           sep = ",", #This command creates the first separation at the first comma
           extra = "merge", #If there are additional spaces after the first, this command puts them all into the second column
           remove = FALSE) #This command keeps the original column

# Woohoo! Got it! AND I understand it! This assignment rocks!
...
mpg_model_class <- mpg %>% #This line creates a new dataset based on the mpg data frame
  unite(col = mpg_model_class, #And creates a new column named "mpg_model_class"
        c(model, class), #The new column combines the 'model' and 'class' columns
        sep = "-", #This line puts a space between the two variables once they're combined
        remove = FALSE #This keeps the original column
        )

#YESSSSS, this rocks SO MUCH!
...
ymd("2020/11/03") #This recodes 2020/11/03 to a YYYYMMDD format

ymd("2020-11-03") #This recodes 2020-11-03 to a YYYYMMDD format

ymd(20201103) #This recodes 20201103 to a YYYYMMDD format

#I kept getting an error that says "All formats failed to parse. No formats found."Much easier to troubleshoot this stuff when it's in small little chunks...I realized that I wasn't putting the appropriate quotation marks around my dates. Problem solved!
...
```{r}
# Your code here: 
some_dates <- c(03121988, 08091989, 070888) #this stores 3 different values as vectors
some_dates_converted <- mdy(some_dates) #this converts these values to a MMDDYYYY format
some_dates_converted #this calls up the new dates
...
gasol_hookshots <- lakers %>% 
  filter(player == "Pau Gasol", type == "hook") %>%  #This is me filtering to only include hook shots scored by Pau Gasol.
  mutate(date_new = ymd(date)) %>%  #This is me creating a new column formatting the date like YYYY-MM-DD
  mutate(month_new = month(date_new)) #This is me creating a new column just showing the value of the month

hook_by_month <- gasol_hookshots %>% #this creates a new dataset based on "gasol_hookshots"
    group_by(month_new, type) %>% #this tells R that we're going to have 2 columns titled 'month_new' and 'type'
    count(month_new) #This line says that there will be a third column displaying the number of times Gasol attempted a hook shot in a given month  

##OK, I got it, and I'm proud, but I still feel like I should somehow be using the summarize function? And also, I'm not totally sure how to get the name of the last column- "n"- to show up as something else. I've tried mutating a few things, which don't really work...so I'm going to call this a win and move on.
...
storms_danger <- storms %>% #Start from storms data
  mutate(danger = case_when( #Add a new column named "danger"
    status == c("hurricane") ~ "high", #This states that if the variable in the column 'status' is EQUAL TO 'hurricane', that the associated value in the new 'danger' column will be 'high'
    TRUE ~ "moderate" #This line says that any variables in the 'status' column NOT matching 'hurricane' will have the associated value 'moderate' in the new 'danger' column
    ))
